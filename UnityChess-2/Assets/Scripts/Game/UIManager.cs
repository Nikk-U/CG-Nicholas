...
public class UIManager : MonoBehaviourSingleton<UIManager> {
    ...
    [SerializeField] private Button startHostButton = null;
    // Button to start the client.
    [SerializeField] private Button startClientButton = null;
    // Button to join a session.
    [SerializeField] private Button joinSessionButton = null;
    // Button to leave a session.
    [SerializeField] private Button leaveSessionButton = null;
    // Button to rejoin a session.
    [SerializeField] private Button rejoinSessionButton = null;
    // Text element to display connection status messages.
    [SerializeField] private Text connectionStatusText = null;
    ...
    private void Start() {
        ...
        rejoinSessionButton.onClick.AddListener(OnRejoinSessionClicked);

        playerConnectionManager = FindObjectOfType<PlayerConnectionManager>();
        playerConnectionManager.ConnectionStatusChanged += OnConnectionStatusChanged;
    }
    ...
    /// <summary>
    /// Handles the start host button click event.
    /// </summary>
    ...
    /// <summary>
    /// Handles the start client button click event.
    /// </summary>
    ...
    /// <summary>
    /// Handles the join session button click event.
    /// </summary>
    private void OnJoinSessionClicked() {
        string sessionCode = "exampleSessionCode"; // Replace with actual session code input
        playerConnectionManager.JoinSession(sessionCode);
        Debug.Log("Client joined the session.");
    }

    /// <summary>
    /// Handles the leave session button click event.
    /// </summary>
    private void OnLeaveSessionClicked() {
        playerConnectionManager.LeaveSession();
        Debug.Log("Client left the session.");
    }

    /// <summary>
    /// Handles the rejoin session button click event.
    /// </summary>
    private void OnRejoinSessionClicked() {
        playerConnectionManager.RejoinSession();
        Debug.Log("Client rejoined the session.");
    }
}